...
void CSkinDialog::SetDBClickMax(bool bMax)
{
	m_bLDBExtMax = bMax;
}
...
BOOL CSkinDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case 5002://最大化？？？
		{
			CSkinButton* btnMax = (CSkinButton*)GetDlgItem(5002);
			m_bMAX = !m_bMAX;
			if (m_bMAX)
			{
				if (btnMax)					//更换按钮
					btnMax->SetBackImage(TEXT("\\Res\\Button\\btn_restore_normal.png"), TEXT("\\Res\\Button\\btn_restore_highlight.png"), TEXT("\\Res\\Button\\btn_restore_down.png"), TEXT("\\Res\\Button\\btn_restore_normal.png"));
			...
			else
			{
				//更换按钮
				if (btnMax)
					btnMax->SetBackImage(TEXT("\\Res\\Button\\btn_max_normal.png"), TEXT("\\Res\\Button\\btn_max_highlight.png"), TEXT("\\Res\\Button\\btn_max_down.png"), TEXT("\\Res\\Button\\btn_max_normal.png"));
...
{
	tagString* pStr = GetString(uID);
	CString sIDStringText = _T("");
	if(pStr)
		sIDStringText = pStr->strText;
	CBuildDialog::SetString(uID, strText);

	CRect rcClient;
	GetClientRect(&rcClient);

	tagPositionData* pPositionData = GetStringRect(uID);
	if (pPositionData && (!strText.IsEmpty() || !sIDStringText.IsEmpty()))
	{
		//CRect rcString;
		//int nTop = pPositionData->ptPosition.y;
		//int nLeft = pPositionData->ptPosition.x;
		//rcString.SetRect(nTop, nLeft,
		//	nLeft + pPositionData->szSize.cx, nTop + pPositionData->szSize.cx);
		CRect rcBounder;
		UIRenderEngine->GetRect(m_hOwnWnd, pPositionData, rcBounder);
		InvalidateRect(rcBounder);
	}
}

void CSkinDialog::ShowString(UINT uID, BOOL bShow)
{
	CBuildDialog::ShowString(uID, bShow);
	tagPositionData* pPositionData = GetStringRect(uID);
	if (pPositionData)
	{
		//CRect rcString;
