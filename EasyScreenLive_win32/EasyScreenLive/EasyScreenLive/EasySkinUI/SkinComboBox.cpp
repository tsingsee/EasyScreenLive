...
LRESULT CSkinComboBox::OnCtlColorListBox( WPARAM wParam, LPARAM lParam )
{
	...
	if ( hListBoxWnd == NULL) 
	{
		...
		if (hWnd != 0 && hWnd != m_hWnd) 
		{
			hListBoxWnd = hWnd;
#ifdef _WIN64
			m_pWndProc = (WNDPROC)GetWindowLongPtr(hListBoxWnd, GWL_WNDPROC);
			SetWindowLongPtr(hListBoxWnd, GWL_WNDPROC, (LONG_PTR)ComboBoxListBoxProc);
#else
...
{
	if( !Create(WS_CHILD|WS_VISIBLE|CBS_DROPDOWN  |WS_VSCROLL | WS_TABSTOP,CRect(0,0,0,0),pParentWnd,0) )
		return FALSE;
