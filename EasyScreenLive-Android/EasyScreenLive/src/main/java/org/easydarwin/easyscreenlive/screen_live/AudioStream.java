...
class AudioStream {
    ...
    private EasyAudioStreamCallback mAudioStreamCb;
    ...
    public AudioStream(EasyAudioStreamCallback audioStreamCb) {
        this.mAudioStreamCb = audioStreamCb;
        int i = 0;
    ...
    private void init() {
        try {
            stoped=false;
            mPushAudio = false;
            mAudioEncCodec = AudioCodec.EASY_SDK_AUDIO_CODEC_AAC;
                ...
                while (!Thread.interrupted() && !stoped) {
                    try {
                        ...
                        while (!Thread.currentThread().isInterrupted() && !stoped) {
                            final int tmpIndex = mMediaCodec.dequeueOutputBuffer(mBufferInfo, 50000);
                            mIndex = tmpIndex;
                            if (mIndex >= 0) {
...
//                            mAudioStreamCb.pushFrame(mChannelId, JniEasyScreenLive.FrameFlag.EASY_SDK_AUDIO_FRAME_FLAG, System.currentTimeMillis(), mBuffer.array(), 0, size);
                            mAudioStreamCb.audioDataBack(System.currentTimeMillis(), mBuffer.array(), 0, size);
                        }
//                        if(mIndex > 0) {
